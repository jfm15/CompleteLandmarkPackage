...
def alpha_angles(ax, points, color):
    ...
    predicted_points = np.take(points, important_indices, axis=0)
    ...
    # left
    ax.plot([centers[0, 0], predicted_points[0, 0]], [centers[0, 1], predicted_points[0, 1]], color=color)
    ax.plot([centers[0, 0], predicted_points[1, 0]], [centers[0, 1], predicted_points[1, 1]], color=color)

    # right
    ax.plot([centers[1, 0], predicted_points[5, 0]], [centers[1, 1], predicted_points[5, 1]], color=color)
    ax.plot([centers[1, 0], predicted_points[6, 0]], [centers[1, 1], predicted_points[6, 1]], color=color)
...
def lce_angles(ax, points, color):
    ...
    centers = []
    for i in [10, 25]:
        centers.append(get_center_of_circle(points[i],
                                            points[i + 1],
                                            points[i + 2]).numpy())
    centers = np.array(centers)

    # left
    ax.plot([centers[0, 0], points[0, 0]], [centers[0, 1], points[0, 1]], color=color)
    ax.plot([centers[0, 0], centers[0, 0]], [centers[0, 1], centers[0, 1] - 50], color=color)

    # right
    ax.plot([centers[1, 0], points[15, 0]], [centers[1, 1], points[15, 1]], color=color)
    ax.plot([centers[1, 0], centers[1, 0]], [centers[1, 1], centers[1, 1] - 50], color=color)
